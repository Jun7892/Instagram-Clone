...
def comment_delete():
    ...
    print(db.comments.find_one({'num': int(num_receive)})['id'])
    print(request.form['id_give'])
    if (request.form['id_give'] == db.comments.find_one({'num': int(num_receive)})['id']) :
        db.comments.delete_one({'num': int(num_receive)})
        return jsonify({'msg': '댓글 삭제 완료'})
    else : return jsonify({'msg': '본인의 댓글만 삭제 가능합니다.'})
