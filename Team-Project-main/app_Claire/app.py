...
ca = certifi.where()
app = Flask(__name__)
client = MongoClient('mongodb+srv://test:sparta@cluster0.kkzda.mongodb.net/Cluster0?retryWrites=true&w=majority', tlsCAFile=certifi.where())
db = client.dbsparta
...
@app.route('/')
def home():
    return render_template('main.html') # 일단 연습예제
...
def homework_get():
    c_list = list(db.comment.find({}, {'_id': False}))
    return jsonify({'cheer': c_list})
